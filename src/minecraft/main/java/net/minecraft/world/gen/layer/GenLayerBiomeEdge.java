    ...
    public int[] getInts(int areaX, int areaY, int areaWidth, int areaHeight)
    {
        ...
        for (int i = 0; i < areaHeight; ++i)
        {
            for (int j = 0; j < areaWidth; ++j)
            {
                ...
                if (!this.replaceBiomeEdgeIfNecessary(aint, aint1, j, i, areaWidth, k, Biome.getIdForBiome(Biomes.MOUNTAINS), Biome.getIdForBiome(Biomes.MOUNTAIN_EDGE)) && !this.replaceBiomeEdge(aint, aint1, j, i, areaWidth, k, Biome.getIdForBiome(Biomes.WOODED_BADLANDS_PLATEAU), Biome.getIdForBiome(Biomes.BADLANDS)) && !this.replaceBiomeEdge(aint, aint1, j, i, areaWidth, k, Biome.getIdForBiome(Biomes.BADLANDS_PLATEAU), Biome.getIdForBiome(Biomes.BADLANDS)) && !this.replaceBiomeEdge(aint, aint1, j, i, areaWidth, k, Biome.getIdForBiome(Biomes.GIANT_TREE_TAIGA), Biome.getIdForBiome(Biomes.TAIGA)))
                {
                    if (k == Biome.getIdForBiome(Biomes.DESERT))
                    {
                        ...
                        {
                            aint1[j + i * areaWidth] = Biome.getIdForBiome(Biomes.WOODED_MOUNTAINS);
                        }
                    }
