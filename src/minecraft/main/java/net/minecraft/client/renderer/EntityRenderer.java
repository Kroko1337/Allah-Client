...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    private void orientCamera(float partialTicks)
    {
        ...
        this.cloudFog = this.mc.worldRenderer.hasCloudFog(d0, d1, d2, partialTicks);
    }
        ...
        if (this.lightmapUpdateNeeded)
        {
            ...
            if (world != null)
            {
                ...
                for (int i = 0; i < 256; ++i)
                {
                    ...
                    float f3 = world.dimension.getLightBrightnessTable()[i % 16] * (this.torchFlickerX * 0.1F + 1.5F);
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano)
    {
        ...
        if (pass == 0 || pass == 2)
        {
            ...
            this.mc.worldRenderer.updateChunks(finishTimeNano);
        }
    ...
    private void addRainParticles()
    {
        ...
        if (f != 0.0F)
        {
            ...
            for (int l = 0; l < k; ++l)
            {
                ...
                if (blockpos1.getY() <= blockpos.getY() + 10 && blockpos1.getY() >= blockpos.getY() - 10 && biome.canRain() && biome.getTemperatureRaw(blockpos1) >= 0.15F)
                {
    ...
    private void renderItemActivation(int widthsp, int heightScaled, float partialTicks)
    {
        if (this.itemActivationItem != null && this.itemActivationTicks > 0)
        {
            ...
            GlStateManager.disableCull();
            RenderHelper.enableStandardItemLighting();
            GlStateManager.translate((float)(widthsp / 2) + f5 * MathHelper.abs(MathHelper.sin(f4 * 2.0F)), (float)(heightScaled / 2) + f6 * MathHelper.abs(MathHelper.sin(f4 * 2.0F)), -50.0F);
