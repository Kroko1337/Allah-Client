...
public class NoiseGeneratorOctaves extends NoiseGenerator
{
    ...
    private final int octavesint;

    public NoiseGeneratorOctaves(Random seed, int octavesIn)
    {
        this.octavesint = octavesIn;
        this.octaves = new NoiseGeneratorImproved[octavesIn];

        for (int i = 0; i < octavesIn; ++i)
        {
            this.octaves[i] = new NoiseGeneratorImproved(seed);
        }
    }
    ...
    public double[] generateNoiseOctaves(double[] noiseArray, int xOffset, int yOffset, int zOffset, int xSize, int ySize, int zSize, double xScale, double yScale, double zScale)
    {
        ...
        for (int j = 0; j < this.octavesint; ++j)
        {
            ...
            d0 = d0 + (double)k;
            d2 = d2 + (double)l;
            this.octaves[j].populateNoiseArray(noiseArray, d0, d1, d2, xSize, ySize, zSize, xScale * d3, yScale * d3, zScale * d3, d3);
            d3 /= 2.0D;
        }
