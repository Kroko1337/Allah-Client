...
public class BlockStateContainer
{
    ...
    {
        return MoreObjects.toStringHelper(this).add("block", Block.REGISTRY.getKey(this.owner)).add("properties", Iterables.transform(this.properties.values(), GET_NAME_FUNC)).toString();
    }
    ...
    static class StateImplementation extends BlockStateBase
    {
        ...
        public <T extends Comparable<T>> T get(IProperty<T> property)
        {
            ...
            if (comparable == null)
            {
                throw new IllegalArgumentException("Cannot get property " + property + " as it does not exist in " + this.block.getStateContainer());
            }
            else
            {
        ...
        public <T extends Comparable<T>, V extends T> IBlockState withProperty(IProperty<T> property, V value)
        {
            ...
            if (comparable == null)
            {
                throw new IllegalArgumentException("Cannot set property " + property + " as it does not exist in " + this.block.getStateContainer());
            }
            ...
            else
            {
                ...
                if (iblockstate == null)
                {
                    throw new IllegalArgumentException("Cannot set property " + property + " to " + value + " on block " + Block.REGISTRY.getKey(this.block) + ", it is not an allowed value");
                }
                else
                {
                    return iblockstate;
        ...
        {
            return this.block.getMaterialColor(this, worldIn, pos);
        }

        public IBlockState rotate(Rotation rot)
        {
            return this.block.rotate(this, rot);
        }

        public IBlockState mirror(Mirror mirrorIn)
        {
            return this.block.mirror(this, mirrorIn);
        }
