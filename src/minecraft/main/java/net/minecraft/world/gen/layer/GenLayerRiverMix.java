    ...
    public int[] getInts(int areaX, int areaY, int areaWidth, int areaHeight)
    {
        ...
        for (int i = 0; i < areaWidth * areaHeight; ++i)
        {
            if (aint[i] != Biome.getIdForBiome(Biomes.OCEAN) && aint[i] != Biome.getIdForBiome(Biomes.DEEP_OCEAN))
            {
                ...
                {
                    if (aint[i] == Biome.getIdForBiome(Biomes.SNOWY_TUNDRA))
                    {
                        aint2[i] = Biome.getIdForBiome(Biomes.FROZEN_RIVER);
                    }
                    else if (aint[i] != Biome.getIdForBiome(Biomes.MUSHROOM_FIELDS) && aint[i] != Biome.getIdForBiome(Biomes.MUSHROOM_FIELD_SHORE))
                    {
                        aint2[i] = aint1[i] & 255;
                    }
                    else
                    {
                        aint2[i] = Biome.getIdForBiome(Biomes.MUSHROOM_FIELD_SHORE);
                    }
                }
                else
                {
