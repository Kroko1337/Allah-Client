...
public class RenderGlobal implements IWorldEventListener, IResourceManagerReloadListener {
    ...
    public RenderGlobal(Minecraft mcIn) {
        ...
        this.skyVertexFormat.addElement(new VertexFormatElement(0, VertexFormatElement.EnumType.FLOAT, VertexFormatElement.EnumUsage.POSITION, 3));
        this.generateStars();
        this.generateSky();
        this.generateSky2();
    }
    ...
    private boolean isOutlineActive(Entity entityIn, Entity viewer, ICamera camera) {
        boolean flag = viewer instanceof EntityLivingBase && ((EntityLivingBase) viewer).isSleeping();
    ...
    public void renderSky(float partialTicks, int pass) {
        if (this.mc.world.dimension.getType().getId() == 1) {
            ...
            double d3 = this.mc.player.getEyePosition(partialTicks).y - this.world.getHorizon();
    ...
    public void renderClouds(float partialTicks, int pass, double x, double y, double z) {
        if (this.mc.world.dimension.isSurfaceWorld()) {
            if (this.mc.gameSettings.shouldRenderClouds() == 2) {
                ...
                double d5 = (double) ((float) this.ticks + partialTicks);
                ...
                float f6 = this.world.dimension.getCloudHeight() - (float) y + 0.33F;
    ...
    private void renderCloudsFancy(float partialTicks, int pass, double x, double y, double z) {
        ...
        float f = 12.0F;
        float f1 = 4.0F;
        double d3 = (double) ((float) this.ticks + partialTicks);
        double d4 = (x + d3 * 0.029999999329447746D) / 12.0D;
        double d5 = z / 12.0D + 0.33000001311302185D;
        float f2 = this.world.dimension.getCloudHeight() - (float) y + 0.33F;
    ...
    public void spawnParticle(int id, boolean ignoreRange, boolean minimiseParticleLevel, final double x, final double y, final double z, double xSpeed, double ySpeed, double zSpeed, int... parameters) {
        try {
            ...
            crashreportcategory.addDetail("ID", Integer.valueOf(id));

            if (parameters != null) {
                crashreportcategory.addDetail("Parameters", parameters);
            }
    ...
    private Particle spawnParticle0(int particleID, boolean ignoreRange, boolean minParticles, double xCoord, double yCoord, double zCoord, double xSpeed, double ySpeed, double zSpeed, int... parameters) {
        ...
        if (this.mc != null && entity != null && this.mc.particles != null) {
            ...
            if (ignoreRange) {
                return this.mc.particles.spawnEffectParticle(particleID, xCoord, yCoord, zCoord, xSpeed, ySpeed, zSpeed, parameters);
            } else if (d3 * d3 + d4 * d4 + d5 * d5 > 1024.0D) {
                return null;
            } else {
                return k1 > 1 ? null : this.mc.particles.spawnEffectParticle(particleID, xCoord, yCoord, zCoord, xSpeed, ySpeed, zSpeed, parameters);
            }
        } else {
            return null;
        }
    ...
    public void playEvent(EntityPlayer player, int type, BlockPos blockPosIn, int data) {
        ...
        switch (type) {
            ...
            case 2001:
                ...
                this.mc.particles.addBlockDestroyEffects(blockPosIn, block.getStateFromMeta(data >> 12 & 255));
                break;
