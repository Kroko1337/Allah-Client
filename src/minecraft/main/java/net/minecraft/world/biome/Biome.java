...
public abstract class Biome
{
    ...
    protected Biome(Biome.BiomeProperties properties)
    {
        ...
        this.depth = properties.baseHeight;
        this.scale = properties.heightVariation;
        this.temperature = properties.temperature;
        this.downfall = properties.rainfall;
        this.waterColor = properties.waterColor;
        this.enableSnow = properties.enableSnow;
        this.enableRain = properties.enableRain;
        this.parent = properties.baseBiomeRegName;
    ...
    {
        double d0 = (double)MathHelper.clamp(this.getTemperatureRaw(pos), 0.0F, 1.0F);
            ...
            else
            {
                ...
                else if (iblockstate2.getBlock() == Blocks.STONE)
                {
                    if (j == -1)
                    {
                        ...
                        {
                            if (this.getTemperatureRaw(blockpos$mutableblockpos.setPos(x, j1, z)) < 0.15F)
                            {
                                iblockstate = ICE;
                            }
                            else
                    ...
                    else if (j > 0)
                    {
                        ...
                        if (j == 0 && iblockstate1.getBlock() == Blocks.SAND && k > 1)
                        {
                            ...
                            iblockstate1 = iblockstate1.get(BlockSand.VARIANT) == BlockSand.EnumType.RED_SAND ? RED_SANDSTONE : SANDSTONE;
                        }
                    }
                }
            }
    ...
    private static void registerBiome(int id, String name, Biome biome)
    {
        ...
        if (biome.isMutation())
        {
            MUTATION_TO_BASE_ID_MAP.put(biome, getIdForBiome(REGISTRY.getOrDefault(new ResourceLocation(biome.parent))));
        }
    }
