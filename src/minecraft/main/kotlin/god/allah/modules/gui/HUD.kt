...
class HUD : Module() {
    ...
    override fun onEvent(event: Event) {
        when (event) {
            is Render2DEvent -> {
                var yAxis = 3
                fr.drawStringWithShadow("Allah", (event.resolution.width / 2 - fr.getStringWidth("Allah") / 2).toFloat(), fr.FONT_HEIGHT.toFloat(), ThreadLocalRandom.current().nextInt(0x0000000, 0x9999999))
